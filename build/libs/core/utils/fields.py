...
from django.db.models import SlugField, DateTimeField
...
class CreateDateTimeField(DateTimeField):
    """
    A DateTimeField that timestamps itself when the object is created
    If there is a preset value on first save, it doesn't overwrite it
    """
    def __init__(self, *args, **kwargs):
    ...
    def pre_save(self, model, add):
        if add:
            if getattr(model, self.attname) is not None:
...
class ChangeDateTimeField(CreateDateTimeField):
    """
    A DateTimeField that timestamps itself every time the object is saved
    If there is a preset value on first save, it doesn't overwrite it
    """
    def pre_save(self, model, add):
        if add and getattr(model, self.attname) is not None:
            value = getattr(model, self.attname)
        else:
            value = datetime.now()
            setattr(model, self.attname, value)
